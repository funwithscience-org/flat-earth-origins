...
2. THE VICTORIAN ANCESTOR IS CARPENTER, NOT ROWBOTHAM, AND HE IS EXPLICIT.
   *One Hundred Proofs* (1885, Gutenberg #55387) carries the argument form twice, in
   its own index: proof 42 "Projectiles--firing east or west" ("there would
   unquestionably be a difference in the result of firing off a projectile in that
   direction and in a direction the opposite of that one. But as, in fact, there is not
   the slightest difference…"), restated with a figure at proof 44 "Firing in opposite
   direction", and proof 54 "Balloons not left behind". Proof 44 verbatim: "since the Earth is said to move at
...
3. THE AEROPLANE VERSION: GABRIELLE HENRIET. Dubay's *200 Proofs* proof 26 is a block
   ...
   earth does not rotate", on the spread paginated 10–11: an aircraft's distance covered "would be reduced
...
<p>What it does not do is rescue the two claims actually on the list. <em>&ldquo;Flight times symmetric&rdquo;</em> is false in the published timetables whatever defines the inertial frame, and <em>&ldquo;aircraft navigation not compensating for spin&rdquo;</em> is false in the alignment procedure whatever is rotating relative to what &mdash; a procedure that derives latitude from a sensed rotation is compensating for one, on any account of what is going round. A defender who reaches for general covariance to save those two sentences has conceded them.</p>""",
    ...
    sources=[
        dict(label="Gabrielle Henriet, Heaven and Earth (translated from the French; mid-1950s) — "
                   "ch. II “On the fact that the earth does not rotate”, the spread paginated 10–11, "
                   "the aircraft "
