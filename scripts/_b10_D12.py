...
4. THE LEDGER IS IN HIS OWN TABLE OF CONTENTS. Sections IV, V, VI, VII, VIII, IX and XIII
   each supply a separate mechanism for an appearance the received account gets from one
   tilted Earth turning as it orbits. That is the parsimony count, and Rowbotham wrote it.
...
6. THE HISTORICAL INVERSION IS THE HEADLINE AND IT IS DOCUMENTED IN TWO TRANSLATIONS.
   Ptolemy — the geocentric authority the same list cites at items 21 and 23 — gives the
   rotating-Earth hypothesis the phrase "this simpler conjecture" in Almagest I.7 and
   rejects it on TERRESTRIAL physics, not on economy. Verified in the unattributed
   English text at bertie.ccsu.edu and, independently, in Toomer's wording as quoted by
   the Max Planck MPRL study. Copernicus runs the same criterion the other way (I.6).
...
"D12": dict(
    ...
    tldr=("Simplicity is a preference, not a measurement, and no count that would settle it "
          "appears in either of the two summary chapters of Rowbotham searched here. Made "
          "under the measure he invited, the count runs the other way: his own table of "
          "contents gives sunrise, the seasons and the Sun's changing path a separate "
          "mechanism each, where one tilted Earth turning as it orbits delivers all three. "
          "Ptolemy had already called the rotating-Earth account “this simpler conjecture”, "
          "granted it the appearances of the stars and rejected it on other grounds — and the "
...
<p>And the count, when someone finally makes it, does not come out his way. It comes out against him on his own contents page, and against the geocentric branch on a bill it pays in a different currency &mdash; a universe that has to turn, and a mechanism that has to make the turning invisible to a gyroscope. Both are worked below.</p>"""),

    refutation="""<p><strong>First, what would have to be true for this to be an argument.</strong> Parsimony does two jobs in science, and neither is the one asked of it here. It breaks ties between rivals that predict <em>the same observations</em>, and it penalises free parameters in model selection, where the penalty is derived from how badly an over-flexible model predicts new data. Both jobs need a stated measure &mdash; simplicity of what, counted how &mdash; because there is no theory-neutral scale on which a description is simply Simpler. The Stanford Encyclopedia&rsquo;s survey of the problem is a good tour of why. No such measure, and no count under one, is offered in the two summary chapters searched for this entry: Section XIV of the 1865 edition and Chapter XV of the 1881 third edition. What is offered is an adjective.</p>

<p><strong>Second, the ledger, and it is Rowbotham&rsquo;s.</strong> The 1865 book&rsquo;s own table of contents assigns a separate chapter, and a separate mechanism, to each of: the Sun moving in a circle over the Earth concentric with the North Pole (IV); the diameter of the Sun&rsquo;s path constantly changing (V); the cause of day and night and the seasons (VI); the cause of sunrise and sunset (VII); the cause of the Sun appearing larger when rising and setting (VIII); the cause of eclipses (IX); the tides (X); the Moon&rsquo;s phases (XII); and perspective on the sea (XIII). Take the six that both accounts have to explain. In the received one, IV, V, VI and VII are a single fact &mdash; a tilted Earth turning once a day while it goes round the Sun &mdash; and IX and XII are two consequences of one Moon in one orbit. Six stipulations against two, with VIII, X and XIII still to be paid for separately on the plane, while on the globe the tides fall out of the gravitation the orbits already needed. That is a count, under a stated measure &mdash; independently stipulated mechanisms &mdash; and it is the measure this argument itself invites by complaining about &ldquo;assumed general motions&rdquo;.</p>
...
<p><strong>Verdict: not demonstrated.</strong> Parsimony is asserted here, and the argument that would license it &mdash; a measure, a count under it, and a reason why the simpler description of a set of appearances should be the true account of what is moving &mdash; is not supplied in the chapters searched for this entry. Where a measure is supplied and a count is made, it goes against the claim in both branches: six stipulated mechanisms against two on Rowbotham&rsquo;s own contents page, and a bought cosmological entity in the geocentric one. And the conclusion does not rest on that, because the case for the Earth&rsquo;s motion has never rested on elegance. It rests on aberration, on parallax, on the ring laser and on the barycentric correction, and those are measurements, which is the currency both sides of this argument said they wanted to be paid in.</p>""",

    advocate=dict(
        best_defense=(
            "You spend one paragraph saying parsimony is not evidence and then five arguing "
            "that our model is the less parsimonious one. You do think it is evidence. You "
            "just want to be holding it. Second, your count is a choice of metric wearing a "
            ...
            "rhetoric with arithmetic in it. Third, you concede that Copernicus was not "
            ...
            "elegance, and you are the one who brought a ledger. Fourth, Rowbotham's actual "
        ...
        survives=4,
        preemptive=(
            "Four, and the number is driven by the second and fourth moves. Four concrete "
            ...
            "rather than re-arguing them; keep it that way."),
    ),
    ...
    sources=[
        ...
        dict(label="Toomer's rendering of the same Almagest I.7 sentence, quoted in the Max Planck "
                   "MPRL study of Aristotle's and Ptolemy's approaches to geocentrism — an "
                   "independent check on the wording",
