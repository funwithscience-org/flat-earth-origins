...
5. THE HEDGE-DROP IS DOCUMENTED TO THE SENTENCE. The book's footnote quotes
   ...
   OCR loss. Same footnote quotes the preprint's 5-sigma / 2.5e-7 figure; the
   peer-reviewed PLB paper's axis-free number is 7.9e-4, about 3.16 sigma (that
   conversion is Patel & Desmond's, their Table 1).
...
"E05": dict(
    ...
    tldr=("Spiral galaxies really were found to prefer one handedness on one side of the "
          ...
          "consistent with zero in his own data, and every independent re-analysis since 2008 "
          "— including one that took a rival 4.0σ claim down to 0.29σ by deleting duplicated "
          "galaxies — finds the sky consistent with isotropy."),
...
<p><strong>Verdict: refuted.</strong> Not because the measurement was fabricated &mdash; it was not &mdash; but because the inference fails twice over. Even at full strength the observable is an axis, and an axis fixes a direction that every observer in the universe shares; the source&rsquo;s own citation says this in a figure caption and the source prints and dismisses the same point from a science journalist two pages earlier. And the measurement is not at full strength: the significance halves when the axis is not assumed, the opposite-hemisphere half is consistent with zero in the originating dataset, the axis sits nearer the survey&rsquo;s own footprint than to any cosmic landmark, and the two decades of re-analysis since have gone one way.</p>""",
