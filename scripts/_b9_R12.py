...
5. NUMBERS, ALL FROM THE PAPERS AND CHECKABLE. Zhang & Stebbins: ACT 95% upper limit
   dT^2 < 8 uK^2 at l = 3000 (SPT < 13 uK^2); Hubble-bubble models within 3 sigma of the
   UNION2 supernova fit predict dT_H^2 > 10^3 uK^2 at the same multipole, "two orders of
   magnitude larger"; models tuned to pass the kSZ bound then fail the supernovae with
   delta-chi^2 > 195; only voids with Omega_0 > ~0.8 or radius < ~0.6 h^-1 Gpc survive;
   the model-independent bound is |dH(z)/H(z)| < ~1% per mass shell of radial width
   ~1 h^-1 Gpc. Ellis's own "typical observationally viable model" has the observer
   "within 10% of the central position" of a void of size 160-250 h^-1 Mpc.
...
7. VERDICT KEPT AT REFUTED, AND MISLEADING WAS SERIOUSLY CONSIDERED. The bare proposition
   ...
   voids below ~0.6 h^-1 Gpc survive. Reported in record_problems.
"""
...
<p><strong>Fourth, the measurement, with the numbers.</strong> A universe that violates the Copernican principle radially &mdash; a deep void with us near the middle, big enough to fake cosmic acceleration &mdash; makes distant matter move relative to the background radiation, and that relative motion prints a kinetic Sunyaev&ndash;Zel&rsquo;dovich signal on the small-angle microwave sky. Zhang and Stebbins computed it. The observed ceiling is the ACT 95% upper limit, &Delta;<em>T</em>&sup2; &lt; 8 &micro;K&sup2; at multipole &#8467; = 3000 (SPT gives &lt; 13 &micro;K&sup2;). Void models within 3&sigma; of the UNION2 supernova fit predict &Delta;<em>T</em>&sup2; above 10&sup3; &micro;K&sup2; &mdash; two orders of magnitude over the ceiling &mdash; and the models retuned to slip under it then miss the supernovae by &Delta;&chi;&sup2; &gt; 195. Stated model-independently, the deviation of any ~1 <em>h</em>&#8315;&sup1; Gpc shell from the overall expansion is held to about 1%. The principle stopped being a postulate and became a number with an error bar, and that is the specific measurement the verdict rests on.</p>
...
<p><strong>Verdict: refuted, on the source&rsquo;s claim rather than the item&rsquo;s.</strong> &ldquo;It is an assumption&rdquo; was true when written and is conceded here without reservation. &ldquo;It is philosophy dressed as science, and it decides the data in advance&rdquo; is the claim the book actually makes, and it is contradicted by a specific measurement: the microwave background&rsquo;s small-angle power spectrum, which bounds radial inhomogeneity at the percent level per gigaparsec and excludes by two orders of magnitude the only version of a non-Copernican universe anybody has built. The foundation was checked, as the author they quote asked. It held.</p>""",
    ...
    sources=[
        ...
        dict(label="Zhang & Stebbins, “Confirmation of the Copernican principle at Gpc radial "
                   ...
                   "PRL 107:041301 (2011) — ACT limit ΔT² < 8 µK² at ℓ = 3000; void models "
                   "overshoot by two orders of magnitude; |ΔH/H| ≲ 1% per ~1 h⁻¹ Gpc shell",
