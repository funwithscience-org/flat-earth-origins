...
<p>There is a second reason the kernel does not deliver, and it decides this cluster. Even granting operationalism in full, the argument fails <em>on its own evidence</em>. If the practical primacy of Earth-fixed coordinates were evidence of a fixed Earth, we would expect those coordinate systems to require no rotation parameter. Instead every one of them has Earth's rotation rate written into its definition, and several break by tens of metres if that rate is set to zero. The items point at the artefacts; the artefacts contain the answer.</p>"""),
    refutation="""<p>Twenty-eight items is the largest cluster on the list, and the first useful observation is that it is one idea in twenty-eight vocabularies. WGS84, ITRF, ECEF, ECI, barycentric, ephemeris, topocentric, pseudorange, great-circle: the technical range creates an impression of independent lines of evidence converging, but every item has the identical logical form &mdash; <em>a practical system uses Earth-fixed coordinates, therefore the Earth is fixed</em>. Establishing that is half the work, because 28 restatements of one invalid inference are not 28 pieces of evidence. They sort into seven kinds.</p>
...
<p>It does not deliver consistent results. The standard compilation files Isaiah 40:22 under &ldquo;Earth is a Disk/Circle&rdquo; and Revelation 7:1 under &ldquo;Earth has 4 Corners/Quarters&rdquo; &mdash; two different shapes, entered as two separate proofs of the same conclusion. Its pillars heading reads, verbatim, &ldquo;Earth has Pillars, and hangs on nothing,&rdquo; bundling Job 26:7 with 1 Samuel 2:8 and Job 9:6 under a single label that states two incompatible things. A method that returns &ldquo;circle&rdquo; and &ldquo;four corners&rdquo; and &ldquo;pillars&rdquo; and &ldquo;hangs upon nothing&rdquo; as four independent confirmations of one physical model is not filtering anything. It is counting.</p>
